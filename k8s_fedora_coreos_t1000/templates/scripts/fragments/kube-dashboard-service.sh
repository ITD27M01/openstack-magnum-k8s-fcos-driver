...
if [ "$(echo $KUBE_DASHBOARD_ENABLED | tr '[:upper:]' '[:lower:]')" == "true" ]; then
    KUBE_DASH_IMAGE="$(_set_prefix 'docker.io/kubernetesui/')dashboard:${KUBE_DASHBOARD_VERSION}"
    HEAPSTER_IMAGE="$(_set_prefix 'gcr.io/google_containers/')heapster-${ARCH}:v1.4.2"
    METRICS_SCRAPER_IMAGE="$(_set_prefix 'docker.io/kubernetesui/')metrics-scraper:${METRICS_SCRAPER_TAG}"
...
EOF
    ...
    if [ "$(echo $INFLUX_GRAFANA_DASHBOARD_ENABLED | tr '[:upper:]' '[:lower:]')" == "true" ]; then
        ...
        INFLUX_IMAGE="$(_set_prefix 'gcr.io/google_containers/')heapster-influxdb-${ARCH}:v1.3.3"
        GRAFANA_IMAGE="$(_set_prefix 'gcr.io/google_containers/')heapster-grafana-${ARCH}:v4.4.3"
