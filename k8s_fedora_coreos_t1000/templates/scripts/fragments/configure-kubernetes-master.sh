...
_prefix=$(_set_prefix "k8s.gcr.io/")
...
ExecStartPre=-/bin/bash -c '/usr/bin/podman run --privileged --user root --net host --entrypoint /bin/bash --rm --volume /usr/local/bin:/host/usr/local/bin ${_prefix}hyperkube:\${KUBE_TAG} -c "cp /usr/local/bin/kubectl /host/usr/local/bin/kubectl"'
...
EOF
    ...
    _prefix=$(_set_prefix "docker.io/openstackmagnum/")
...
KUBE_CONTROLLER_MANAGER_CONFIG=/etc/kubernetes/controller-manager-config.yaml

cat > ${KUBE_CONTROLLER_MANAGER_CONFIG} << EOF
apiVersion: v1
clusters:
- cluster:
    certificate-authority: ${CERT_DIR}/ca.crt
...
_prefix=$(_set_prefix "gcr.io/google_containers/")
KUBELET_ARGS="--register-node=true --pod-manifest-path=/etc/kubernetes/manifests --hostname-override=${INSTANCE_NAME}"
KUBELET_ARGS="${KUBELET_ARGS} --pod-infra-container-image=${_prefix}pause:3.1"
