...
if [ "${new_kube_tag}" != "${KUBE_TAG}" ]; then
    ...
    if [ "$(echo $USE_PODMAN | tr '[:upper:]' '[:lower:]')" == "true" ]; then
        ...
        ${ssh_cmd} podman rmi ${_prefix}hyperkube:${KUBE_TAG}
        ...
        i=0
        until [ "`${ssh_cmd} podman image exists ${_prefix}hyperkube:${new_kube_tag} && echo $?`" = 0 ]
        do
            i=$((i+1))
            [ $i -lt 30 ] || break;
        ...
        KUBE_DIGEST=$($ssh_cmd podman image inspect ${_prefix}hyperkube:${new_kube_tag} --format "{{.Digest}}")
        ...
        for service in ${SERVICE_LIST}; do
            ${ssh_cmd} atomic pull --storage ostree "${_prefix}${service_image_mapping[${service}]}:${new_kube_tag}"
        done

        for service in ${SERVICE_LIST}; do
            ${ssh_cmd} atomic containers update --rebase ${_prefix}${service_image_mapping[${service}]}:${new_kube_tag} ${service}
        done
        ...
        for service in ${SERVICE_LIST}; do
            ${ssh_cmd} atomic --assumeyes images "delete ${_prefix}${service_image_mapping[${service}]}:${KUBE_TAG}"
        done
